...
public class TrainingAppState extends SimulatorAppState {
    ...
    protected void startSimulation(SimulationData data) {
        ...
        if (isFixedTimeStep) {
            brain = new AdvancedAlienBrain(this.physics.getPhysicsSpace().getAccuracy(), this.physics.getSpeed(), this.fixedTimeStep);
            brain.setNN(nn);
        } else {
            brain = new AdvancedAlienBrain(this.physics.getPhysicsSpace().getAccuracy(), this.physics.getSpeed());
            brain.setNN(nn);
        }
